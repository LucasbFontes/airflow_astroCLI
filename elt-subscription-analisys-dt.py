...
def etl_subscriptions_analysis():
 ...
 # init & finish task
    ...
    user_file = aql.load_file(
        task_id="user_file",
        input_file=File(path="s3://airflowfiles/user/", filetype=FileType.JSON, conn_id=S3_CONN_ID),
    ...
    subscription_file = aql.load_file(
        ...
        input_file=File(path="s3://airflowfiles/subscription/", filetype=FileType.JSON, conn_id=S3_CONN_ID),
    ...
    # aql.dataframe = Pandas DataFrame
    # apply logic to find out users by importance -> [pandas dataframe]
    ...
    chain(
        ...
        subscriptions_curated_warehouse,
        finish
    )
